...
public class ConnectionPoolStreamingTest {
    ...
    public static void main(String[] args) {
        try {
            ConnectionPoolStreamingTest test = new ConnectionPoolStreamingTest();
            test.setUp();
            test.streamSingleAudioFile();
        } catch (Exception e) {
            logger.error("Error running example: {}", e.getMessage());
            System.exit(1);
        }
    }
    ...
    void streamSingleAudioFile() throws Exception {
        try {
            ...
            if (!allStreamsCompleteLatch.await(120, TimeUnit.SECONDS)) {
                throw new RuntimeException("Example timed out waiting for streams to complete");
            }
    ...
    private void startNewStream() throws Exception {
        ...
        try {
            ...
            client.setOnError(error -> {
                logger.error("[Conn {}] WebSocket error: {}", connId, error);
            });

            client.setOnOpen(unused -> {
                logger.info("[Conn {}] Connection established", connId);
            });
